...
public interface IRedisService {
    ...
    /**
     * 执行 Lua 脚本（Sha1哈希值）
     * @param luaScript 脚本内容（Sha1哈希值）
     * @param returnType 返回类型
     * @param keys 键
     * @param args 值（ARGS，不能为null）
     * @return 执行结果
     */
    <T> T executeSha(String luaScript, RScript.ReturnType returnType, List<Object> keys, Object... args);

    void incrMap(String key, String field, int delta);

    <K, V> void addAllToMap(String key, Map<K, V> map);
}
