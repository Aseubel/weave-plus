package com.aseubel.common.redis.service;
...
public class RedissonService implements IRedisService {
    ...
    public <T> void setValue(String key, T value, long expired) {
        ...
        bucket.set(value, expired, TimeUnit.MILLISECONDS);
    }
    ...
    public void setSetExpired(String key, long expired) {
        RSet<String> set = redissonClient.getSet(key);
        set.expire(expired, TimeUnit.MILLISECONDS);
    }
    ...
    public void setMapExpired(String key, long expired) {
        RMap<String, String> map = redissonClient.getMap(key);
        map.expire(expired, TimeUnit.MILLISECONDS);
    }
    ...
    public Boolean setNx(String key, Duration expired) {
        return redissonClient.getBucket(key).trySet("lock", expired.toMillis(), TimeUnit.MILLISECONDS);
    }

    @Override
    public <T> T execute(String shaDigest, String luaScript, RScript.ReturnType returnType, List<Object> keys, Object... values) {
        try {
            // 1. 在获取脚本对象时，为其指定 StringCodec
            // 这会覆盖客户端默认的 JsonJacksonCodec，仅对本次操作有效
            RScript script = redissonClient.getScript(StringCodec.INSTANCE);

            // 2. 优先尝试使用 EVALSHA 执行，效率最高
            return script.evalSha(RScript.Mode.READ_WRITE, shaDigest, returnType, keys, values);

        } catch (RedisException e) {
            // 3. 捕获 Redis 异常，并检查是不是 NOSCRIPT 错误
            if (e.getMessage().startsWith("NOSCRIPT")) {
                log.warn("Lua script with SHA {} not found, falling back to EVAL.", shaDigest);

                // 4. 如果是 NOSCRIPT 错误，同样使用带 StringCodec 的脚本对象执行 EVAL
                RScript script = redissonClient.getScript(StringCodec.INSTANCE);
                return script.eval(RScript.Mode.READ_WRITE, luaScript, returnType, keys, values);
            }
            // 5. 如果是其他类型的 Redis 异常，则直接向上抛出
            throw e;
        }
    }

    @Override
    public <T> T executeSha(String luaScript, RScript.ReturnType returnType, List<Object> keys, Object... values) {
        return redissonClient.getScript().evalSha(RScript.Mode.READ_WRITE, luaScript, returnType, keys, values);
    }
    ...
    @Override
    public <K, V> void addAllToMap(String key, Map<K, V> map) {
